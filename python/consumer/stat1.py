...
def statistics_1(session, producer):
    ...
    while (True):
        ...
        pd.DataFrame(list(session.execute(f'SELECT max(LPRD) as last_record FROM "statistics_1";')))['last_record'][0]
        ...
        if (new_data.shape[0] > 0):  # if there is new data
            ...
            for i in range(new_data.shape[0]):
                ...
                ts_in_stat1 = pd.DataFrame(list(
                    session.execute(f'SELECT day FROM "statistics_1" where day=\'{record_ts_1}\' allow filtering;')))
